...
--      select
--          uid,order_date
--      from (select distinct uid from exist_uid_date ) t
--      left join (select generate_series(date '2024-11-11',current_date ,'1 day')::date order_date) t1 on 1=1
-- )
--     select
--         order_date,
...
--     from (
--     select uid,balance,recharge_balance,balance_give,order_date
--     from (
--     select
--         t2.uid,
--         t2.order_date,
--         max(t3.order_date) over (partition by t2.uid,t2.order_date) as lay_date,  --找出每条不存在数据 上一条存在数据的日期
--         t3.order_date as exist_order_date,
--         t3.balance,
--         t3.balance_give,
--         t3.recharge_balance
--     from (
--     -- 找到不存在的条目
--     select t.uid,t.order_date
--     from tmp_completion t left join exist_uid_date t1 on t.uid = t1.uid and t.order_date = t1.order_date
--     where t1.uid is null              -- 找出不存在的数据
--     ) t2 left join exist_uid_date t3 on t2.uid = t3.uid and t2.order_date >= t3.order_date -- 链接存在的数据
--     ) t4 where t4.lay_date = exist_order_date   -- 找出每条不存在数据 上一条存在数据的日期
--     union all
--     select * from exist_uid_date  -- union 上存在的条目
--     ) t5
--     left join user_info on t5.uid = user_info.uid and user_info.register_date <= t5.order_date
