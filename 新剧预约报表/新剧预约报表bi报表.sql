...
select
    ...
    case when count(distinct uid)>0 then sum(case when pay_amt>0 and pay_pv>0 then 1 else 0 end )/count(distinct uid) else 0 end as "预约用户付费率",
    case when total_watch_uv_on_d_date>0 then sum(case when watch_pv>0 and watch_duration_sec>0 then 1 else 0 end)/total_watch_uv_on_d_date else 0 end as "预约播放占比"
from t3
