------------------------------------------
-- file: 商业广告报表_全量_d.sql
-- author: xiaoj
-- time: 2025/5/14 10:59
-- description:
------------------------------------------
set timezone ='UTC-0';
...
select
    ...
    sum(case when event = 191 then 1 else 0 end) as total_unlock_ad_click_uv,
    sum(case when event = 191 then pv else 0 end) as total_unlock_ad_click_pv,
...
where d_date >= '2024-09-01' and d_date <= current_date                -- 测试日期
group by d_date,
         coalesce(area, 'UNKNOWN'),
         coalesce(country_name, 'UNKNOWN'),
         coalesce(lang_name, 'UNKNOWN'),
