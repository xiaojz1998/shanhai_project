...
, tmp_event_log as (
    select
        ...
        , event                                         -- 时间
        , t.device_id                                     -- 设备id
        , case when t.vid is not null then t.vid::text
    ...
    from "app_performance_event_log" t
--     inner join (
--         select
--             to_timestamp(created_at):: date as d_date
--             , device_id
--             , vid::text               -- 取到vid 处理null
--         from "app_performance_event_log"
--         where to_timestamp(created_at):: date = '2025-05-20'::date -- 限制时间
...
--                 , device_id
--                 , vid
--     ) t0 on to_timestamp(created_at):: date = t0.d_date and t.device_id = t0.device_id
--                 and case when t.vid is not null then t.vid::text when substring(t.request_url FROM 'playletId=([0-9]+)' ) is not null then substring(t.request_url FROM 'playletId=([0-9]+)' )
--                     else '未知' end = t0.vid
...
left join tmp_event_log t0
    on t.d_date = t0.d_date and t.device_id = t0.device_id -- 用日期和设备id关联
where t0.ad_source_type = 1
group by t.d_date
--     , country_name
--     , area
--     , country_grade
