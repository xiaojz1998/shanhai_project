...
, new_roi as (
    select
        ...
        , sum(pay_0) 新用户首日充值金额
    from tmp_primary a
    left join tmp_camp_roi b on a.campaign_id = b.campaign_id
...
, drama_total as (
    SELECT COALESCE(c.name,'总计') 语种, b.name 剧名,b.english_name as 剧译名, b.id
