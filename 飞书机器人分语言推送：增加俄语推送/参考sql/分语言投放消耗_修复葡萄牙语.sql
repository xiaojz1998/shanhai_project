...
with newuser_tb as (
  select
    uid,
    if(campaign_id = '0', campaign_name, campaign_id) campaign_id,
    campaign_name,
    country_code,
    put_system,
    v_type,
...
roi_tb as (
  select
    ...
    count(
      distinct if(
        newuser_tb.uid = pay_tb.uid
        and newuser_tb.p_date = pay_tb.p_date,
        newuser_tb.uid,
        null
      )
    ) as new_pay_uv,
    sum(
      if(
        newuser_tb.uid = pay_tb.uid
        and pay_tb.p_date = newuser_tb.p_date,
        pay_tb.money,
        0.0
      )
    ) as money_0
...
total_cost as (
  select
    ...
    当月累计消耗,
    case
      when cost_target is null then 0
      else round(
        当月累计消耗 * 100.0 /(cost_target),
        2
      )
    end 当月累计消耗完成率,
  ...
  from
    (
      select
        ...
        -- remaining_days,
        COALESCE(case
          when remaining_days = 0.0 then 剩余消耗
          else round(
            剩余消耗 / remaining_days,
            2
          )
      ...
      from
        (
          select
            ...
            EXTRACT(
              ...
              FROM
                (
                  DATE_TRUNC('month', t1.p_date) + INTERVAL '1 month' - INTERVAL '1 day'
                ) - t1.p_date
            ) :: numeric AS remaining_days
          from
            (
              select
                cost_tb.p_date,
              ...
              from
                cost_tb
                left join roi_tb on cost_tb.p_date = roi_tb.p_date
                and cost_tb.campaign_id = roi_tb.campaign_id
            ) t1
            left join "oversea-api_osd_videos" vid_tb on cast(split_part(t1.campaign_name, '_', 5) as varchar) = cast(vid_tb.id as varchar)
            left join "oversea-api_osd_categories" language_tb on cast(vid_tb.category_id as varchar) = cast(language_tb.id as varchar)
          where
        ...
        left join total_cost b on a.日期 = b.p_date
        and a.语言 = b.语言
        left join total_recharge c on a.日期 = c.日期
        and a.语言 = c.语言
    ) a
  where
    to_char(
