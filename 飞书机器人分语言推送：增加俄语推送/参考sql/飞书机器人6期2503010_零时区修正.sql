with t27 as (SELECT uid,  min("created_date") as first_pay_date FROM  public.all_order_log --找出付费用户第一个付费日
...
t28 as (SELECT uid,
           CASE
               WHEN (CURRENT_DATE - INTERVAL '1 day') = TO_DATE(first_pay_date::text, 'YYYYMMDD') THEN 1
               ELSE 0
           END AS is_first_pay  -- 计算是否为昨天的首充
    FROM t27),

t29 as (SELECT uid,  min("created_date") as first_k_pay_date FROM  public.all_order_log --找出K币付费用户第一个付费日
where "environment" = 1 --生产环境
and "status"  = 1 --充值成功（没减去退款用户）
and "coin" > 0
AND created_date BETWEEN 20000101 AND 20991231
...
t30 as (SELECT uid,
           CASE
               WHEN (CURRENT_DATE - INTERVAL '1 day') = TO_DATE(first_k_pay_date::text, 'YYYYMMDD') THEN 1
               ELSE 0
           END AS is_first_k_pay  -- 计算是否为昨天的首充
    FROM t29),

t31 as (SELECT uid,  min("created_date") as first_sub_pay_date FROM  public.all_order_log --找出订阅付费用户第一个付费日
where "environment" = 1 --生产环境
and "status"  = 1 --充值成功（没减去退款用户）
...
t32 as (SELECT uid,
           CASE
               WHEN (CURRENT_DATE - INTERVAL '1 day') = TO_DATE(first_sub_pay_date::text, 'YYYYMMDD') THEN 1
               ELSE 0
           END AS is_first_sub_pay  -- 计算是否为昨天的订阅首充
    FROM t31),

t1 as (select distinct uid, created_date from public.all_order_log  -- 选取昨天只充值K币的首充用户
where "environment" = 1 --生产环境
and "coin"  > 0 --充值金币数大于0
and "status"  = 1 --充值成功（没减去退款用户）
...
t2 as (select COUNT(*) as new_k_pay_users,(CASE 
            WHEN lang_name IN ('简体中文', '繁体中文') THEN '中文'
            ELSE lang_name 
        END) AS lang_name from t1  -- 合并繁体和简体中文 lang_name from t1  --首充K币用户数量按语言分类
INNER JOIN public.dwd_user_info u
on t1.uid = u.uid::bigint
GROUP BY (CASE 
            WHEN lang_name IN ('简体中文', '繁体中文') THEN '中文'
            ELSE lang_name 
        END)),

t3 as (select sum(dau) as dau, (CASE 
            WHEN lang_name IN ('简体中文', '繁体中文') THEN '中文'
            ELSE lang_name 
        END) AS lang_name from  public.dw_operate_view a
...
t4 as (select new_k_pay_users, dau, (CASE 
            WHEN t2.lang_name IN ('简体中文', '繁体中文') THEN '中文'
            ELSE t2.lang_name 
        END) AS lang_name from t2 -- 全连接DAU表和首充K币表
FULL OUTER JOIN  t3 
on t2.lang_name = t3.lang_name),

t5 as (select distinct uid, created_date from public.all_order_log --首充订阅会员用户
where "environment" = 1 --生产环境
...
t6 as (select COUNT(*) as new_vip_pay_users, (CASE 
            WHEN u.lang_name IN ('简体中文', '繁体中文') THEN '中文'
            ELSE u.lang_name 
        END) AS lang_name from t5  --首充订阅会员用户数量按语言分类
INNER JOIN public.dwd_user_info u
on t5.uid = u.uid::bigint
GROUP BY (CASE 
            WHEN u.lang_name IN ('简体中文', '繁体中文') THEN '中文'
            ELSE u.lang_name 
        END)),  
  
t7 as (select sum(pay_amt+ad_income_amt) as "总收入(含商广)" ,(CASE 
...
t8 as (select CASE WHEN SUM((all_pay_user))  = 0 THEN 0 ELSE
COALESCE(SUM((pay_user*1.0)) / SUM(all_pay_user * 1.0),0)
END as "订单转化率：UV", (CASE 
            WHEN a.lang_name IN ('简体中文', '繁体中文') THEN '中文'
            ELSE a.lang_name 
        END) AS lang_name from public.dw_operate_view a
where  TO_DATE(a.d_date, 'YYYY-MM-DD') = CURRENT_DATE - INTERVAL '1 day'
...
t9 as (select (case when sum(due_user)=0 then 0.00 
...
t10 as (select (CASE WHEN  SUM(pay_user_dau) = 0 THEN 0
ELSE 
SUM(pay_user_2login) * 1.0 / SUM(pay_user_dau) * 1.0
END) as "总付费次留", (CASE 
            WHEN a.lang_name IN ('简体中文', '繁体中文') THEN '中文'
            ELSE a.lang_name 
        END) AS lang_name from public.dw_operate_view a
...
t11 as (select (CASE WHEN  SUM(new_pay_user_dau) = 0 THEN 0
ELSE 
SUM(new_pay_user_2login) * 1.0 / SUM(new_pay_user_dau) * 1.0
END) as "新用户付费次留", (CASE 
            WHEN a.lang_name IN ('简体中文', '繁体中文') THEN '中文'
            ELSE a.lang_name 
        END) AS lang_name from public.dw_operate_view a
...
t12 as (select (CASE WHEN  SUM(old_pay_user_dau) = 0 THEN 0
ELSE 
SUM(old_pay_user_2login) * 1.0 / SUM(old_pay_user_dau) * 1.0
END) as "老用户付费次留", (CASE 
            WHEN a.lang_name IN ('简体中文', '繁体中文') THEN '中文'
            ELSE a.lang_name 
        END) AS lang_name from public.dw_operate_view a
...
t13 as (select (CASE WHEN  SUM(dau) = 0 THEN 0
ELSE 
SUM(dau_2login) * 1.0 / SUM(dau) * 1.0
END) as "总次留", (CASE 
            WHEN a.lang_name IN ('简体中文', '繁体中文') THEN '中文'
            ELSE a.lang_name 
        END) AS lang_name from public.dw_operate_view a
...
t14 as (select (CASE WHEN  SUM(new_dau) = 0 THEN 0
ELSE 
SUM(new_dau_2login) * 1.0 / SUM(new_dau) * 1.0
END) as "新用户次留", (CASE 
            WHEN a.lang_name IN ('简体中文', '繁体中文') THEN '中文'
            ELSE a.lang_name 
        END) AS lang_name from public.dw_operate_view a
...
t15 as (select (CASE WHEN  SUM(old_dau) = 0 THEN 0
ELSE 
SUM(old_dau_2login) * 1.0 / SUM(old_dau) * 1.0
END) as "老用户次留", (CASE 
            WHEN a.lang_name IN ('简体中文', '繁体中文') THEN '中文'
            ELSE a.lang_name 
        END) AS lang_name from public.dw_operate_view a
...
t16 as (select CASE WHEN SUM((all_pay_order))  = 0 THEN 0 ELSE
...
, t18 as ( select TO_TIMESTAMP(created_at) :: date active_date , uid  -- 活跃表
...
select TO_TIMESTAMP(created_at) :: date active_date, uid
from user_log
where true 
and created_date >= 20240801
group by TO_TIMESTAMP(created_at) :: date,uid )

, t19 as ( select active_date,COALESCE(register_date,'2024-07-01') register_date
 , COALESCE(area,'未知') area, COALESCE(国家,'未知') 国家, COALESCE(系统,'Android') 系统, COALESCE(campaign_id,'0') campaign_id, t18.uid
...
FROM t19 a left join t19 b 
on a.uid = b.uid and b.active_date > a.active_date
INNER JOIN public.dwd_user_info u
on a.uid = u.uid::bigint
where a.active_date = CURRENT_DATE - INTERVAL '4 day'
group by a.active_date, (CASE 
            WHEN u.lang_name IN ('简体中文', '繁体中文') THEN '中文'
            ELSE u.lang_name 
        END)),
...
t23 as (select t20.lang_name, "总3留率", "总7留率", "总30留率"
from t20
inner join t21
...
t24 as (select distinct created_date, uid  from public.all_order_log o
...
t25 as (select COUNT(*) as pay_again_users_num, (CASE 
            WHEN u.lang_name IN ('简体中文', '繁体中文') THEN '中文'
            ELSE u.lang_name 
        END) as lang_name from t24  --复充会员用户数量按语言分类（第二次及以上）
INNER JOIN public.dwd_user_info u
on t24.uid = u.uid::bigint
GROUP BY (CASE 
            WHEN u.lang_name IN ('简体中文', '繁体中文') THEN '中文'
            ELSE u.lang_name 
        END)),
...
t26 as (select sum(pay_user) as pay_user_num, (CASE 
            WHEN lang_name IN ('简体中文', '繁体中文') THEN '中文'
            ELSE lang_name 
        END) as lang_name from public.dw_operate_view 
where TO_DATE(d_date, 'YYYY-MM-DD') = CURRENT_DATE - INTERVAL '1 day'
group by (CASE 
            WHEN lang_name IN ('简体中文', '繁体中文') THEN '中文'
            ELSE lang_name 
...
t33 as (SELECT COUNT(DISTINCT a.uid) as pay_again_users_num , (CASE       -----老充值用户在昨日复充人数
