...
-- 注入用户分层表
-----------------------------------------------
truncate table tmp.dw_user_layer_tag_tmp01 ;
insert into tmp.dw_user_layer_tag_tmp01
    ...
    tmp_total_mg_report_active_record as (
        ...
        select
            *,
            '2' as app_source_id
        from mg_xingju_report_active_record
    ),
	...
	tmp_device_guiyin as(
	    ...
	    )t2 on t1.link_id=t2.link_id and t1.app_source_id = t2.app_source_id
	)
	-- 广告解锁集数、观看剧集数
	,tmp_ad_unlock as(
		select device_id
	...
	insert into dw.dw_user_layer_tag select * from tmp.dw_user_layer_tag_tmp01;
