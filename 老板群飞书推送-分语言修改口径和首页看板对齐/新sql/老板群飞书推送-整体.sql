...
with t as (
    ...
    from (
        ...
        group by d_date :: date
    ) t
    left join (select d_date::date as 日期, sum(active_uv) as dau from public.ads_rpt_home_page_hi group by d_date) t0 on t.日期 = t0.日期
),
t0 as (
    -- 维度： 日期
    -- 功能： 获取每天的广告收入
