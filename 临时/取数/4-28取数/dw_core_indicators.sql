...
WITH weekly_date_ranges AS (
    ...
    weekly_start AS (
        SELECT
            date_trunc('week', date + INTERVAL '1 day')::date
            AS week_start
        FROM date_series
        GROUP BY week_start
    )
...
operate_view_summary AS (
    ...
    GROUP BY week_start, week_end, country_name, lang_name
),
...
retention_summary AS (
    ...
    GROUP BY week_start, week_end, 国家,lang_name
),
...
schedule_summary AS (
    ...
    JOIN weekly_date_ranges
    ON d_date::date BETWEEN week_start AND week_end
    GROUP BY week_start, week_end, country_name
    ,lang_name
),
...
weekly_stats AS (
    ...
    GROUP BY DATE_TRUNC('week', TO_TIMESTAMP(a.created_at)::date), b.country_name, COALESCE(nn.lang_name, 'UNKNOWN')
)
SELECT
    ws.week_start,
    ws.week_end,
    ws.国家,
    ws.语言,
    SUM(ws.新增绑定账号用户数) OVER (PARTITION BY ws.国家, ws.语言 ORDER BY ws.week_start ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS 绑定账号的累计注册用户数,
    SUM(ws.总用户数) OVER (PARTITION BY ws.国家, ws.语言 ORDER BY ws.week_start ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS 累计注册用户数
FROM weekly_stats ws
ORDER BY ws.week_start
)

SELECT
    COALESCE(o.week_start, ov.week_start, r.week_start, p.week_start, s.week_start, t.week_start, nw.week_start, nub.week_start) AS week_start,
    COALESCE(o.week_end, ov.week_end, r.week_end, p.week_end, s.week_end, t.week_end, nw.week_end, nub.week_end) AS week_end,
...
FULL JOIN retention_summary r
    ON o.week_start = r.week_start AND o.week_end = r.week_end AND o.国家 = r.国家 AND o.语言 = r.语言
FULL JOIN push_summary p
    ON o.week_start = p.week_start AND o.week_end = p.week_end AND o.国家 = p.国家 AND o.语言 = p.语言
FULL JOIN schedule_summary s
    ON o.week_start = s.week_start AND o.week_end = s.week_end AND o.国家 = s.国家 AND o.语言 = s.语言
FULL JOIN task_summary t
    ON o.week_start = t.week_start AND o.week_end = t.week_end AND o.国家 = t.国家 AND o.语言 = t.语言
FULL JOIN newuser_watch_summary nw
    ON o.week_start = nw.week_start AND o.week_end = nw.week_end AND o.国家 = nw.国家 AND o.语言 = nw.语言
FULL JOIN newuser_bind_summary nub
    ON o.week_start = nub.week_start AND o.week_end = nub.week_end AND o.国家 = nub.国家 AND o.语言 = nub.语言
ORDER BY COALESCE(o.week_start, ov.week_start, r.week_start, p.week_start, s.week_start, t.week_start, nw.week_start, nub.week_start) desc
