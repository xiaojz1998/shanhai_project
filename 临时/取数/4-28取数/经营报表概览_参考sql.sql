------------------------------------------
-- file: 经营报表概览_参考sql.sql
-- author: xiaoj
-- time: 2025/4/29 16:04
-- description:
------------------------------------------
set timezone ='UTC-0';
	    ...
	    ,tmp_dau as(
			...
			where d_date >= (current_date+interval '-31 day')
	    	...
	    	from(
		    	...
		    	from(
		    		...
		    		where d_date between '2024-07-01' and current_date::text
		    			and d_date >= (current_date+interval '-31 day')::date::text -- 增
		    	)d
		...
		----
		,t0.os
		,coalesce(t8.cps_money,0) as cps_money
	    from tmp_primary t0
	    left join(
	        select
	         u1.created_date
		...
		,coalesce(u0.os,'UNKNOWN') as os
	        ...
	        and o.created_date>=20250101
	        	and to_char( to_timestamp(o.created_at),'YYYY-MM-DD') >= (current_date+interval '-31 day')::date::text -- 增
		...
		left join(
			...
			and t1.begin_date>='2024-07-01'
				and t1.begin_date >= (current_date+interval '-31 day')::date::text -- 增
		...
		left join(
			...
			and t1.end_date>='2024-07-01'
				and t1.end_date >= (current_date+interval '-31 day')::date::text -- 增
	    ...
	    left join(
	        ...
	        and r.refund_date>=20250101
	        	and to_char( to_timestamp(r.refund_time),'YYYY-MM-DD') >= (current_date+interval '-31 day')::date::text -- 增
	    ...
	    left join(
			...
			and cd.created_date>=20250101
				and cd.created_date >= to_char((current_date+interval '-31 day'),'yyyymmdd')::int  -- 增
	    ...
	    left join(
			...
			and a.created_date::int >=20250101
				and a.created_date::int >= to_char((current_date+interval '-31 day'),'yyyymmdd')::int  -- 增
		...
		left join (
			...
			,lang
			,os
			,sum(cps_money) as cps_money
			from public.dw_operate_cps
			group by d_date,area,country_code,lang,os
		) t8 on t0.d_date=t8.d_date and t0.area=t8.area and t0.country_code=t8.country_code and t0.lang=t8.lang and t0.os=t8.os
	    ;
