...
with excluded_data as (
    ...
    WHERE a.event = 2 AND a.vid > 0 AND a.eid > 0 AND to_timestamp(a.created_at)::date >= '2025-03-01'
    GROUP BY to_timestamp(a.created_at):: date,a.uid ) t2
        on t1.d_date::date = t2.d_date::date and t1.uid::bigint = t2.uid::bigint
    where t1.d_date>= '2025-03-01' and( watch_duration_sec is null or watch_duration_sec < 3)
...
tmp_primary as(
    ...
    from analysis.dim_day where d_date between '2025-03-01' and '2025-04-30'
),
-- 未排除
user_pay_status AS (
    SELECT DISTINCT
...
tmp_t3 as (
     select
	        ...
	        where 1=1 and ed.uid is null and o.environment = 1 and o.os in('android','ios')
...
select
    t0.d_date as "日期",
    (sum(t3.pay_amt) + sum(t7.ad_income_amt))/100 as "收入",
    sum(t1.dau) as dau,
...
where t0.d_date >= '2025-03-01'::date and t0.d_date <= '2025-04-30'::date
group by t0.d_date
