set timezone ='UTC-0';
		...
		delete from tmp.dw_operate_view_tmp01 where d_date>= (current_date+interval '-2 day')::date::text;
    ...
    --   AND order_type IN (1, 4, 5)
      ...
      AND status = 1
	    ...
	    ;
			...
			delete from public.dw_operate_view where d_date>= (current_date+interval '-2 day')::date::text;
			insert into public.dw_operate_view select * from tmp.dw_operate_view_tmp01 where d_date>= (current_date+interval '-2 day')::date::text;
