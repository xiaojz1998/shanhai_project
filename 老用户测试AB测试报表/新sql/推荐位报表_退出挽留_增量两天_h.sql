...
aggregated_k_consume as (
    ...
    and cd.event = 191
    group by cd.date, cd.user_type, cd.country_name, cd.ad_channel, cd.user_group,cd.show_title, cd.lang_name, cd.registration_period
),
aggregated_exposure AS (
    ...
    FROM exposure_data
    GROUP BY date, user_type, show_title, country_name, ad_channel, user_group,lang_name,registration_period
),
aggregated_click AS (
    SELECT
        date,
...
aggregated_payment AS (
    ...
    GROUP BY pd.date, pd.user_type, pd.show_title, pd.country_name, pd.ad_channel, pd.user_group,pd.lang_name,pd.registration_period
)
SELECT
    ae.date,
    ae.user_type,
...
LEFT JOIN aggregated_click ac ON ae.date = ac.date AND ae.user_type = ac.user_type AND ae.show_title = ac.show_title AND ae.country_name = ac.country_name AND ae.ad_channel = ac.ad_channel AND ae.user_group = ac.user_group and ae.lang_name = ac.lang_name and ae.registration_period = ac.registration_period
LEFT JOIN aggregated_payment ap ON ae.date = ap.date AND ae.user_type = ap.user_type AND ae.show_title = ap.show_title AND ae.country_name = ap.country_name AND ae.ad_channel = ap.ad_channel AND ae.user_group = ap.user_group and ae.lang_name = ap.lang_name and ae.registration_period = ap.registration_period
left join aggregated_k_consume akc on ae.date = akc.date and ae.user_type = akc.user_type and ae.show_title = akc.show_title and ae.country_name = akc.country_name and ae.ad_channel = akc.ad_channel and ae.user_group = akc.user_group and ae.lang_name = akc.lang_name and ae.registration_period = akc.registration_period
WHERE ae.user_group IS NOT NULL
ORDER BY ae.date, ae.user_type, ae.show_title, ae.country_name, ae.ad_channel, ae.user_group,ae.lang_name,ae.registration_period;
