------------------------------------------
-- file: 推荐位报表_退出挽留0324.sql
-- author: xiaoj
-- time: 2025/4/27 10:25
-- description:
------------------------------------------
set timezone ='UTC-0';


-- truncate table tmp.dw_recommend_playretain0324_tmp01;
-- INSERT INTO tmp.dw_recommend_playretain0324_tmp01
delete from tmp.dw_recommend_playretain0324_tmp01 where date between (CURRENT_DATE + INTERVAL '-2 day') and (current_date+interval '-1 day');
INSERT INTO tmp.dw_recommend_playretain0324_tmp01
WITH user_registration AS (
    SELECT
        ...
        ad_channel,
        CASE
            WHEN RIGHT(CAST(uid AS VARCHAR), 1) = '1' THEN '对照组'
            WHEN RIGHT(CAST(uid AS VARCHAR), 1) = '5' THEN '实验组'
            ELSE NULL
        END AS user_group
    FROM dwd_user_info
),
...
aggregated_exposure AS (
    ...
    FROM exposure_data
    GROUP BY date, user_type, show_title, country_name, ad_channel, user_group
),
aggregated_click AS (
    SELECT
        date,
...
aggregated_payment AS (
    ...
    GROUP BY pd.date, pd.user_type, pd.show_title, pd.country_name, pd.ad_channel, pd.user_group
)
SELECT
    ae.date,
    ae.user_type,
...
LEFT JOIN aggregated_click ac ON ae.date = ac.date AND ae.user_type = ac.user_type AND ae.show_title = ac.show_title AND ae.country_name = ac.country_name AND ae.ad_channel = ac.ad_channel AND ae.user_group = ac.user_group
LEFT JOIN aggregated_payment ap ON ae.date = ap.date AND ae.user_type = ap.user_type AND ae.show_title = ap.show_title AND ae.country_name = ap.country_name AND ae.ad_channel = ap.ad_channel AND ae.user_group = ap.user_group
WHERE ae.user_group IS NOT NULL
ORDER BY ae.date, ae.user_type, ae.show_title, ae.country_name, ae.ad_channel, ae.user_group;


-- truncate table public.dw_recommend_playretain0324;
--         insert into public.dw_recommend_playretain0324 select * from tmp.dw_recommend_playretain0324_tmp01;
delete from public.dw_recommend_playretain0324 where date between (CURRENT_DATE + INTERVAL '-2 day') and (current_date+interval '-1 day');
        insert into public.dw_recommend_playretain0324 select * from tmp.dw_recommend_playretain0324_tmp01 where date between (CURRENT_DATE + INTERVAL '-2 day') and (current_date+interval '-1 day');
...
---端内
-- set timezone ='UTC-0';
-- truncate table tmp.dw_recommend_playretain0324_app_tmp01;
-- INSERT INTO tmp.dw_recommend_playretain0324_app_tmp01
delete from tmp.dw_recommend_playretain0324_app_tmp01 where date between (CURRENT_DATE + INTERVAL '-4 day') and (current_date+interval '-1 day');
INSERT INTO tmp.dw_recommend_playretain0324_app_tmp01
WITH user_registration AS (
    SELECT
        d_date AS register_date,
...
-- truncate table public.dw_recommend_playretain0324_app;
-- insert into public.dw_recommend_playretain0324_app select * from tmp.dw_recommend_playretain0324_app_tmp01;
delete from public.dw_recommend_playretain0324_app where date between (CURRENT_DATE + INTERVAL '-4 day') and (current_date+interval '-1 day');
 insert into public.dw_recommend_playretain0324_app select * from tmp.dw_recommend_playretain0324_app_tmp01 where date between (CURRENT_DATE + INTERVAL '-4 day') and (current_date+interval '-1 day');
...
truncate table tmp.dw_recommend_retain0324_index_tmp01;
INSERT INTO  tmp.dw_recommend_retain0324_index_tmp01
WITH
user_selection AS (
    SELECT DISTINCT
           uid,
...
--    to_timestamp(created_at)::date >='2025-03-18'
     to_timestamp(created_at)::date between '2025-03-25' and  (CURRENT_DATE - INTERVAL '1 day')
...
successful_recharges AS (
  ...
  WHERE to_timestamp(created_at)::date >= fe.first_exposure_date
    --   AND to_timestamp(created_at)::date >='2025-03-18'
      and to_timestamp(created_at)::date between '2025-03-25' and (CURRENT_DATE - INTERVAL '1 day')
      AND status = 1
      AND environment = 1
  GROUP BY a.uid
  , 日期
...
user_type_info AS (
    ...
    LEFT JOIN user_selection ur ON d.uid = ur.uid
     WHERE d.d_date BETWEEN '2025-03-25' AND (current_date+interval '-1 day')
    and d.user_type='newuser'
)
SELECT
       cd.type as 实验分组,
...
ORDER BY cd.type;
        ...
        truncate table public.dw_recommend_retain0324_index;
        insert into public.dw_recommend_retain0324_index select * from tmp.dw_recommend_retain0324_index_tmp01;