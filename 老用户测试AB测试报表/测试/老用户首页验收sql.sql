...
aggregated_exposure AS (
    ...
    FROM exposure_data
    GROUP BY p_date, recommendation_id, group_type, res_type
),
aggregated_click AS (
    SELECT
        a.p_date,
    ...
    GROUP BY a.p_date, a.recommendation_id, a.group_type, b.res_type
),
aggregated_payment AS (
    ...
    GROUP BY pd.p_date, cd.recommendation_id, cd.group_type, b.res_type
),
aggregated_k AS (
    SELECT
        a.p_date,
    ...
    GROUP BY a.p_date, cd.recommendation_id, cd.group_type, b.res_type
)
...
SELECT
    ...
    SUM(COALESCE(ae.exposure_times, 0)) AS exposure_times,
    SUM(COALESCE(ac.click_times, 0)) AS click_times,
    SUM(COALESCE(ac.play_times, 0)) AS play_times,
...
GROUP BY ae.p_date, ae.recommendation_id, rc.chinese_name, ae.group_type
