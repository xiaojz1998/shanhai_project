...
WITH k_info AS (
    ...
    SELECT data_date,            -- 日期
           ...
           balance,              -- K币余额
           recharge_coin,        -- 总充值K币
           balance_give_expired, --赠送金币过期数量
           total_balance_give,   -- 累计赠送K币总额
           balance_give          -- 赠送金币余额
    ...
    WHERE status = 1),
     ...
     bal_info as (
         ...
         group by k1.data_date,
                  ...
                  k1.country_name,
                  k1.user_type)
select bal_info.data_date                                                                                 AS "日期",
       bal_info.area                                                                                      AS "区域",
       ...
       COALESCE(coin_give, 0) + COALESCE(coin_num, 0) + COALESCE(gift_num, 0) +
       COALESCE(activity_num, 0)                                                                          AS "累计赠送K币",
...
from bal_info
         left join(SELECT k_info.data_date,
                          COALESCE(
                                  country_info.area,
                                  '未知'
                              )                 AS area,
                   ...
                   FROM k_info
                       ...
                       AND user_info.register_date <= k_info.data_date
                            ...
                            LEFT JOIN(SELECT k_info.uid,
                                      ...
                                      FROM k_info
                                               LEFT JOIN (SELECT uid,
                                                          ...
                                                          WHERE order_type = 1
                                                            AND status = 1
                                                            AND environment = 1) AS recharge_info
                                                         ON k_info.uid = recharge_info.uid
                                                             AND recharge_info.order_date <= k_info.data_date
                                      GROUP BY k_info.uid,
                                               k_info.data_date) AS recharge_coin_give_info
                                     ON k_info.uid = recharge_coin_give_info.uid
                            ...
                            LEFT JOIN(SELECT k_info.uid,
                                      ...
                                      FROM k_info
                                               LEFT JOIN (SELECT uid,
                                                          ...
                                                          WHERE order_type = 1
                                                            AND status = 1
                                                            AND environment = 1) AS day_recharge_info
                                                         ON k_info.uid = day_recharge_info.uid
                                                             AND day_recharge_info.order_date = k_info.data_date
                                      GROUP BY k_info.uid,
                                               k_info.data_date) AS recharge_coin_info
                                     ON k_info.uid = recharge_coin_info.uid
                            ...
                            LEFT JOIN (SELECT k_info.uid,
                                              ...
                                              sum(
                                                      CASE
                                                          WHEN TYPE IN (3, 5, 6, 7, 8, 9, 10, 11, 13, 14, 19, 20)
                                                              THEN reward_coin
                                                          ELSE 0
                                                          END
                                                  ) AS activity_num6
                                       FROM k_info
                                                LEFT join (SELECT c.task_date,
                                                                  c.uid,
                                                                  TYPE,
                                                                  reward_coin
                                                           FROM (SELECT uid,
                                                                 ...
                                                                 FROM "oversea-api_osd_user_task" --任务用户关联表
                                                                 WHERE status = 3) c
                                                                    LEFT JOIN (SELECT id,
                                                                                      TYPE,
                                                                                      --活动类型
                                                                                      reward_coin --奖励K币数
                                                                               FROM "oversea-api_osd_task" --任务配置表
                                                                               WHERE status = 1) d
                                                                              ON c.task_id = d.id) AS activity_info
                                                          ON k_info.uid = activity_info.uid
                                                              AND activity_info.task_date <= k_info.data_date
                                       GROUP BY k_info.uid,
                                                k_info.data_date) AS activity_coin_give_info
                                      ON k_info.uid = activity_coin_give_info.uid
                            ...
                            LEFT JOIN(SELECT k_info.uid,
                                      ...
                                      FROM k_info
                                               LEFT join (SELECT uid,
                                                          ...
                                                          FROM
                                                              --用户签到表
                                                              "oversea-api_osd_user_sign_record") AS sign_info
                                                         ON k_info.uid = sign_info.uid
                                                             AND sign_info.sign_date <= k_info.data_date
                                      GROUP BY k_info.uid,
                                               k_info.data_date) AS sign_coin_give_info
                                     ON k_info.uid = sign_coin_give_info.uid
                                         AND sign_coin_give_info.data_date = k_info.data_date
                            LEFT JOIN (SELECT k_info.uid,
                                              k_info.data_date,
                                              sum(gift_num) AS gift_num
                                       FROM k_info
                                                LEFT JOIN (SELECT e.gift_date,
                                                                  e.uid,
                                                                  coin AS gift_num
                                                           FROM (SELECT uid,
                                                                        gift_code_id,
                                                                        to_timestamp(created_at) :: date AS gift_date
                                                                 FROM "oversea-api_osd_user_gift_code_log" --用户金币兑换记录表
                                                                ) e
                                                                    LEFT JOIN (SELECT id,
                                                                                      coin --单次兑换金额
                                                                               FROM "oversea-api_osd_gift_code" --社媒金币赠送活动表
                                                                               WHERE status = 1) f
                                                                              ON e.gift_code_id = f.id) AS gift_info
                                                          ON k_info.uid = gift_info.uid
                                                              AND gift_info.gift_date <= k_info.data_date
                                       GROUP BY k_info.uid,
                                                k_info.data_date) AS gift_coin_give_info
                                      ON k_info.uid = gift_coin_give_info.uid
                            ...
                            left join(SELECT k_info.uid,
                                      ...
                                      FROM k_info
                                               LEFT JOIN (SELECT uid,
                                                          ...
                                                          WHERE TYPE = 0) AS use_info ON k_info.uid = use_info.uid
                                          AND use_info.order_date <= k_info.data_date
                                      GROUP BY k_info.uid,
                                               k_info.data_date) as use_give_info ON k_info.uid = use_give_info.uid
                   ...
                   GROUP BY k_info.data_date,
                            ...
                            user_info.user_type) as dd on bal_info.data_date = dd.data_date
