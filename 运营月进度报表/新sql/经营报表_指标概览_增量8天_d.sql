set timezone ='UTC-0';
		...
		delete from tmp.dw_operate_view_tmp01 where d_date>= (current_date+interval '-8 day')::date::text;
	    ...
	    ,tmp_dau as(
			...
			where d_date >= (current_date+interval '-8 day')
	    	...
	    	from(
		    	...
		    	from(
		    		...
		    		where d_date between '2024-07-01' and current_date::text
		    			and d_date >= (current_date+interval '-8 day')::date::text -- 增
		    	)d
      ...
      AND status = 1
		...
		,coalesce(u0.os,'UNKNOWN') as os
	        ...
	        and o.created_date>=20250101
	        	and to_char( to_timestamp(o.created_at),'YYYY-MM-DD') >= (current_date+interval '-8 day')::date::text -- 增
		...
		left join(
			...
			and t1.begin_date>='2024-07-01'
				and t1.begin_date >= (current_date+interval '-8 day')::date::text -- 增
		...
		left join(
			...
			and t1.end_date>='2024-07-01'
				and t1.end_date >= (current_date+interval '-8 day')::date::text -- 增
	    ...
	    left join(
	        ...
	        and r.refund_date>=20250101
	        	and to_char( to_timestamp(r.refund_time),'YYYY-MM-DD') >= (current_date+interval '-8 day')::date::text -- 增
	    ...
	    left join(
			...
			and cd.created_date>=20250101
				and cd.created_date >= to_char((current_date+interval '-8 day'),'yyyymmdd')::int  -- 增
	    ...
	    left join(
			...
			and a.created_date::int >=20250101
				and a.created_date::int >= to_char((current_date+interval '-8 day'),'yyyymmdd')::int  -- 增
	    ...
	    ;
			...
			delete from public.dw_operate_view where d_date>= (current_date+interval '-8 day')::date::text;
			insert into public.dw_operate_view select * from tmp.dw_operate_view_tmp01 where d_date>= (current_date+interval '-8 day')::date::text;
