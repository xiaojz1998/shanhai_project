...
, DAU
, lag(DAU) over(partition by a.语言 order by 日期 ) DAU2
-- , 新用户次留
-- , lag(新用户次留) over(partition by a.语言 order by 日期 ) 新用户次留2
, 总次留
, lag(总次留) over(partition by a.语言 order by 日期 ) 总次留2
, 总3留
, lag(总3留) over(partition by a.语言 order by 日期 ) 总3留2
, 总7留
, lag(总7留) over(partition by a.语言 order by 日期 ) 总7留2
, 总付费率
, lag(总付费率) over(partition by a.语言 order by 日期 ) 总付费率2
, 总ARPPU
, lag(总ARPPU) over(partition by a.语言 order by 日期 ) 总ARPPU2
, 总人均付费次数
, lag(总人均付费次数) over(partition by a.语言 order by 日期 ) 总人均付费次数2
, 订阅会员续订率
, lag(订阅会员续订率) over(partition by a.语言 order by 日期 ) 订阅会员续订率2
, 老用户人均付费次数
, lag(老用户人均付费次数) over(partition by a.语言 order by 日期 ) 老用户人均付费次数2
, "PUSH点击率"
, lag("PUSH点击率") over(partition by a.语言 order by 日期 ) "PUSH点击率2"
, "人均播放时长(分钟)"
, lag("人均播放时长(分钟)") over(partition by a.语言 order by 日期 ) "人均播放时长(分钟)2"
, 人均每日观看集数
, lag(人均每日观看集数) over(partition by a.语言 order by 日期 ) 人均每日观看集数2
, 付费集播放率
, lag(付费集播放率) over(partition by a.语言 order by 日期 ) 付费集播放率2
, 人均剧均免费集观看数
, lag(人均剧均免费集观看数) over(partition by a.语言 order by 日期 ) 人均剧均免费集观看数2
from (SELECT d_month
  , bud_area as 语言
  , bud_payamt as 目标收入
  , cost_target as 目标消耗
