...
SELECT md5(CONCAT(a.week_day,a.active_week,a.area,a.country_name,a.os,a.lang_name)) as id
    ...
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND b.active_date - a.active_date BETWEEN 7 AND 13  AND a.user_source='推广流' THEN b.uid ELSE NULL END) 新推广用户次周留存
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND b.active_date - a.active_date BETWEEN 14 AND 20 AND a.user_source='推广流' THEN b.uid ELSE NULL END) 新推广用户2周留存
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND b.active_date - a.active_date BETWEEN 21 AND 27 AND a.user_source='推广流' THEN b.uid ELSE NULL END) 新推广用户3周留存
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND b.active_date - a.active_date BETWEEN 28 AND 34 AND a.user_source='推广流' THEN b.uid ELSE NULL END) 新推广用户4周留存
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND a.user_source='自然流'  THEN a.uid ELSE NULL END) 新自然用户数
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND b.active_date - a.active_date BETWEEN 7 AND 13  AND a.user_source='自然流' THEN b.uid ELSE NULL END) 新自然用户次周留存
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND b.active_date - a.active_date BETWEEN 14 AND 20 AND a.user_source='自然流' THEN b.uid ELSE NULL END) 新自然用户2周留存
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND b.active_date - a.active_date BETWEEN 21 AND 27 AND a.user_source='自然流' THEN b.uid ELSE NULL END) 新自然用户3周留存
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND b.active_date - a.active_date BETWEEN 28 AND 34 AND a.user_source='自然流' THEN b.uid ELSE NULL END) 新自然用户4周留存
...
from (
    SELECT md5(CONCAT(a.active_month,a.area,a.country_name,a.os,a.lang_name)) as id
        ...
        ,COUNT(DISTINCT CASE WHEN a.active_month = a.register_month AND b.active_date - a.active_date BETWEEN 30 AND 59 AND a.user_source='推广流'  THEN b.uid ELSE NULL END) 新推广用户次月留存
        ,COUNT(DISTINCT CASE WHEN a.active_month = a.register_month AND b.active_date - a.active_date BETWEEN 60 AND 89 AND a.user_source='推广流' THEN b.uid ELSE NULL END) 新推广用户2月留存
        ,COUNT(DISTINCT CASE WHEN a.active_month = a.register_month AND b.active_date - a.active_date BETWEEN 90 AND 119 AND a.user_source='推广流'  THEN b.uid ELSE NULL END) 新推广用户3月留存
        ,COUNT(DISTINCT CASE WHEN a.active_month = a.register_month AND a.user_source='自然流' THEN a.uid ELSE NULL END) 新自然用户数
        ,COUNT(DISTINCT CASE WHEN a.active_month = a.register_month AND b.active_date - a.active_date BETWEEN 30 AND 59 AND a.user_source='自然流'  THEN b.uid ELSE NULL END) 新自然用户次月留存
        ,COUNT(DISTINCT CASE WHEN a.active_month = a.register_month AND b.active_date - a.active_date BETWEEN 60 AND 89 AND a.user_source='自然流' THEN b.uid ELSE NULL END) 新自然用户2月留存
        ,COUNT(DISTINCT CASE WHEN a.active_month = a.register_month AND b.active_date - a.active_date BETWEEN 90 AND 119 AND a.user_source='自然流'  THEN b.uid ELSE NULL END) 新自然用户3月留存
    ...
    FROM tmp.dw_retention_detail_tmp01 a
    LEFT JOIN tmp.dw_retention_detail_tmp01 b ON a.uid = b.uid AND b.week_end > a.week_end
    WHERE a.active_date <= '2025-06-08'
    GROUP BY
        a.active_month
        ,a.area
        ,a.country_name
