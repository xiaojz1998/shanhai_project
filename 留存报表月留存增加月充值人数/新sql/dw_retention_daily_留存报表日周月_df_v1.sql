---------------------------------------------
-- File: dw_retention_daily_留存报表日周月_df_v1.sql
-- Time: 2025/6/11 18:51
-- User: xiaoj
-- Description:  
---------------------------------------------
SET timezone ='UTC';
...
SELECT md5(CONCAT(a.active_date,a.area,a.country_name,a.os,a.lang_name,a.is_paid)) as id
    ...
    ,COUNT(DISTINCT CASE WHEN a.active_date = a.register_date AND a.user_source='推广流' THEN a.uid ELSE NULL END) 新推广用户
    ,COUNT(DISTINCT CASE WHEN a.active_date = a.register_date AND a.user_source='推广流' AND b.active_date - a.active_date = 1 THEN b.uid ELSE NULL END ) 新推广用户次日留存
    ,COUNT(DISTINCT CASE WHEN a.active_date = a.register_date AND a.user_source='推广流' AND b.active_date - a.active_date = 3 THEN b.uid ELSE NULL END) 新推广用户3日留存
    ,COUNT(DISTINCT CASE WHEN a.active_date = a.register_date AND a.user_source='推广流' AND b.active_date - a.active_date = 7 THEN b.uid ELSE NULL END) 新推广用户7日留存
    ,COUNT(DISTINCT CASE WHEN a.active_date = a.register_date AND a.user_source='推广流' AND b.active_date - a.active_date = 14 THEN b.uid ELSE NULL END) 新推广用户14日留存
    ,COUNT(DISTINCT CASE WHEN a.active_date = a.register_date AND a.user_source='推广流' AND b.active_date - a.active_date = 30 THEN b.uid ELSE NULL END) 新推广用户30日留存
    ,COUNT(DISTINCT CASE WHEN a.active_date = a.register_date AND a.user_source='自然流' THEN a.uid ELSE NULL END) 新自然用户数
    ,COUNT(DISTINCT CASE WHEN a.active_date = a.register_date AND a.user_source='自然流' AND b.active_date - a.active_date = 1 THEN b.uid ELSE NULL END) 新自然用户次日留存
    ,COUNT(DISTINCT CASE WHEN a.active_date = a.register_date AND a.user_source='自然流' AND b.active_date - a.active_date = 3 THEN b.uid ELSE NULL END) 新自然用户3日留存
    ,COUNT(DISTINCT CASE WHEN a.active_date = a.register_date AND a.user_source='自然流' AND b.active_date - a.active_date = 7 THEN b.uid ELSE NULL END) 新自然用户7日留存
    ,COUNT(DISTINCT CASE WHEN a.active_date = a.register_date AND a.user_source='自然流' AND b.active_date - a.active_date = 14 THEN b.uid ELSE NULL END) 新自然用户14日留存
    ,COUNT(DISTINCT CASE WHEN a.active_date = a.register_date AND a.user_source='自然流' AND b.active_date - a.active_date = 30 THEN b.uid ELSE NULL END) 新自然用户30日留存
...
SELECT md5(CONCAT(a.week_day,a.active_week,a.area,a.country_name,a.os,a.lang_name)) as id
    ...
    ,COUNT(DISTINCT CASE WHEN b.week_end - a.week_end = 7 THEN b.uid  ELSE NULL END) 总次周留存
    ,COUNT(DISTINCT CASE WHEN b.week_end - a.week_end = 14 THEN b.uid ELSE NULL END) 总2周留存
    ,COUNT(DISTINCT CASE WHEN b.week_end - a.week_end = 21 THEN b.uid ELSE NULL END) 总3周留存
    ,COUNT(DISTINCT CASE WHEN b.week_end - a.week_end = 28 THEN b.uid ELSE NULL END) 总4周留存
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week THEN a.uid ELSE NULL END) 新用户数
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND b.week_end - a.week_end = 7 THEN b.uid ELSE NULL END) 新用户次周留存
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND b.week_end - a.week_end = 14 THEN b.uid ELSE NULL END) 新用户2周留存
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND b.week_end - a.week_end = 21  THEN b.uid ELSE NULL END) 新用户3周留存
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND b.week_end - a.week_end = 28  THEN b.uid ELSE NULL END) 新用户4周留存
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND a.user_source='推广流'  THEN a.uid ELSE NULL END) 新推广用户数
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND b.week_end - a.week_end = 7   AND a.user_source='推广流' THEN b.uid ELSE NULL END) 新推广用户次周留存
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND b.week_end - a.week_end = 14  AND a.user_source='推广流' THEN b.uid ELSE NULL END) 新推广用户2周留存
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND b.week_end - a.week_end = 21  AND a.user_source='推广流' THEN b.uid ELSE NULL END) 新推广用户3周留存
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND b.week_end - a.week_end = 28  AND a.user_source='推广流' THEN b.uid ELSE NULL END) 新推广用户4周留存
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND a.user_source='自然流'  THEN a.uid ELSE NULL END) 新自然用户数
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND b.week_end - a.week_end = 7   AND a.user_source='自然流' THEN b.uid ELSE NULL END) 新自然用户次周留存
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND b.week_end - a.week_end = 14  AND a.user_source='自然流' THEN b.uid ELSE NULL END) 新自然用户2周留存
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND b.week_end - a.week_end = 21  AND a.user_source='自然流' THEN b.uid ELSE NULL END) 新自然用户3周留存
    ,COUNT(DISTINCT CASE WHEN a.active_week = a.register_week AND b.week_end - a.week_end = 28  AND a.user_source='自然流' THEN b.uid ELSE NULL END) 新自然用户4周留存
    ,COUNT(DISTINCT CASE WHEN (a.active_week <> a.register_week or a.register_week is null) THEN a.uid ELSE NULL END) 老用户数
    ,COUNT(DISTINCT CASE WHEN (a.active_week <> a.register_week or a.register_week is null) AND b.week_end - a.week_end = 7  THEN b.uid ELSE NULL END) 老用户次周留存
    ,COUNT(DISTINCT CASE WHEN (a.active_week <> a.register_week or a.register_week is null) AND b.week_end - a.week_end = 14  THEN b.uid ELSE NULL END) 老用户2周留存
    ,COUNT(DISTINCT CASE WHEN (a.active_week <> a.register_week or a.register_week is null) AND b.week_end - a.week_end = 21  THEN b.uid ELSE NULL END) 老用户3周留存
    ,COUNT(DISTINCT CASE WHEN (a.active_week <> a.register_week or a.register_week is null) AND b.week_end - a.week_end = 28  THEN b.uid ELSE NULL END) 老用户4周留存
    ,a.lang_name
